...
export const useRecordStore = defineStore('record', () => {
  ...
  async function makeRecord(recordData) {
    ...
    if (group) {
      ...
      try {
        res = await api.get(
          `/groupMembers?userId=${user.id}&groupId=${group.id}`,
        );
      } catch (e) {
  ...
  async function fetchRecord() {
    ...
    if (!group) {
      try {
        const res = await api.get(
          `/records?userId=${user.id}&groupId=${null}&_embed=category&_embed=user`,
        );
    ...
    try {
      const res = await api.get(
        `/records?groupId=${group.id}&_embed=category&_embed=user`,
      );
      records.value = res.data;
    } catch (e) {
      throw new Error('오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
